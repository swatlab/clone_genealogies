...
# VIF analysis
alias(fit3)
formula = update(formula, ~. -EEvPattern -EIncChg -EConStChg -EFltsConStChg -EFltIncStChg)
fit3 = glm(formula, data = df.log, family = binomial())
alias(fit3)
vif(fit3)
formula = update(formula, ~. -TPC -EIncStChg -EConChg -EFltDens)
fit3 = glm(formula, data = df.log, family = binomial())
vif(fit3)
# show coefficients and p-value of M(s+e+g)
fit3
summary(fit3)
# compare M(s+e+g) again M(s+e)
